...
using FastStrap.AppData;
using FastStrap.RobloxInterfaces;
using FastStrap.UI.Elements.Bootstrapper.Base;
...
namespace FastStrap
{
    public class Bootstrapper
    {
        ...
        public string BackgroundUpdaterMutexName => $"FastStrap-BackgroundUpdater-{_launchMode}";

        public string MutexNamePrefix { get; set; } = "FastStrap-Bootstrapper";
        ...
        {
            const string LOG_IDENT = "Bootstrapper::ApplyCloudflareDns";
            try
            {
                var psi = new ProcessStartInfo
                {
                    FileName = "powershell.exe",
                    Arguments = "-NoProfile -NonInteractive -WindowStyle Hidden -Command \"Get-NetAdapter | Where-Object { $_.Status -eq 'Up' } | Set-DnsClientServerAddress -ServerAddresses '1.1.1.1','1.0.0.1'\"",
                    Verb = "runas",
        ...
        {
            const string LOG_IDENT = "Bootstrapper::CloseBackgroundApps";

            var excluded = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
            {
                "explorer", "faststrap", "robloxplayerbeta", "robloxstudiobeta",
                "taskmgr", "regedit", "svchost", "lsass", "csrss", "winlogon",
                "wininit", "services", "smss", "dwm", "sihost", "ctfmon", "fontdrvhost"
            };
            ...
            foreach (var process in targets)
            {
                try
                {
                    App.Logger.WriteLine(LOG_IDENT, $"Closing {process.ProcessName} (PID {process.Id})");
        ...
        private void StartRoblox()
        {
            ...
            if (App.Settings.Prop.UseCloudflareDns)
                ApplyCloudflareDns();

            if (App.Settings.Prop.CloseBackgroundApps)
                CloseBackgroundApps();
            ...
            try
            {
                ...
                if (App.Settings.Prop.RobloxHighPriority)
                {
        ...
        private async Task<bool> CheckForUpdates()
        {
            ...
            {
                App.Logger.WriteLine(LOG_IDENT, $"More than one FastStrap instance running, aborting update check");
                return false;
            }
            ...
            SetStatus(Strings.Bootstrapper_Status_UpgradingFastStrap);

            try
            {
#if DEBUG_UPDATER
                string downloadLocation = Path.Combine(Paths.TempUpdates, "FastStrap.exe");
