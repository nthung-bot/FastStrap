...
using FastStrap.UI.Elements.Bootstrapper;
using FastStrap.UI.Elements.Dialogs;

namespace FastStrap.UI
{
    static class Frontend
    {
        ...
        private static IBootstrapperDialog GetCustomBootstrapper()
        {
            ...
            catch (Exception ex)
            {
                ...
                if (!App.LaunchSettings.QuietFlag.Active)
                    ShowMessageBox(string.Format(Strings.CustomTheme_Errors_SetupFailed, ex.Message, "FastStrap"), MessageBoxImage.Error); // NOTE: FastStrap is the theme name
        ...
        public static void ShowBalloonTip(string title, string message, System.Windows.Forms.ToolTipIcon icon = System.Windows.Forms.ToolTipIcon.None, int timeout = 5)
        {
            ...
            {
                Icon = Properties.Resources.IconFastStrap,
